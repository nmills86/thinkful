# CHANGED VARIABLE NAMES, ATTEMPTED TO CONVERT TO LIST COMPREHENSIONS IN SEVERAL PLACES, BUT IT DIDN'T WORK
...
df = pd.read_sql_query("SELECT country_name, _1999, _2000, _2001, _2002, _2003, _2004, _2005, _2006, _2007, _2008, _2009, _2010, male_exp, female_exp, year FROM gdp INNER JOIN ed_expect ON gdp.country=ed_expect.country_name;",con) # WHAT IS THIS?

# TRANSPOSE
df2 = pd.melt(df, id_vars=['country_name', 'male_exp', 'female_exp', 'year'], value_vars=['_1999','_2000','_2001','_2002','_2003','_2004','_2005','_2006','_2007','_2008','_2009','_2010'],var_name='gdp_year',value_name='gdp')

# CAN'T TURN INTO LIST COMPREHENSION
df2['gdp_year']=df2['gdp_year'].map(lambda x: int(x[1:]))


# ONLY KEEP ROWS WHERE GDP YEAR = EXPECTANCY YEAR
df2=df2[df2.year==df2.gdp_year]
# FILTER OUT BLANK ROWS 
df2=df2[df2.gdp!='']
df2['gdp']=df2['gdp'].map(lambda x: float(x))



log_gdp=np.log(df2['gdp'])
m_exp=df2['male_exp']
f_exp=df2['female_exp']


y=np.matrix(log_gdp).transpose()
male=np.matrix(m_exp).transpose()
female=np.matrix(f_exp).transpose()

Xmale=sm.add_constant(male)
Xfemale=sm.add_constant(female)

# RUN THE MODEL, and EXTRACT FIT STATISTICS, MALE
model = sm.OLS(y,Xmale)
f = model.fit()
